      ...
      results = index
        .filter(function (item) {
          return (
            ...
            item.categories.some(function (c) {
  ...
  function renderResults(results, query) {
    ...
    if (results.length === 0) {
      resultsContainer.innerHTML =
        '<div class="command-palette__empty">没有找到相关结果</div>';
      items = [];
      return;
    }
    ...
    var html = results
      .map(function (item, i) {
        ...
        return (
          '<div class="command-palette__item" data-url="' +
          escapeHtml(item.url) +
          '" data-index="' +
          i +
          '">' +
    ...
    items = Array.from(
      resultsContainer.querySelectorAll(".command-palette__item"),
    );
